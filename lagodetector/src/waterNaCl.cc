...
#include "waterNaCl.hh"


waterNaCl::waterNaCl()
{
  G4cout << "...Creating water with NaCl..." << G4endl;
...
G4Material* waterNaCl::doWater()
{

  O = new G4Element("Oxygen"  , "O", z = 8 , a = 16.00*(g/mole));
  H = new G4Element("Hydrogen", "H", z=1 , a = 1.01*(g/mole));

  Cl = new G4Element("Chlorine", "Cl", z=17, a = 35.45*(g/mole));
  Na = new G4Element("Hydrogen", "Na", z=11, a = 22.99*(g/mole));
  ...
  thewater->AddElement(O, 1);
	...
	theNaCl
		= new G4Material("NaCl", density = 2.16*(g/cm3), 2);
	theNaCl->AddElement(Cl, 1);
	theNaCl->AddElement(Na, 1);

	thewaterNaCl
		= new G4Material("WaterNaCl", density = 1.0*(g/cm3), 2);

	thewaterNaCl->AddMaterial(thewater, 80.0*perCent);
	thewaterNaCl->AddMaterial(theNaCl, 20.0*perCent);

  return thewaterNaCl;
}


G4MaterialPropertiesTable* waterNaCl::doOpwater()
{
